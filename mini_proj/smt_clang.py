...
def rec(c, k, flag=False):
    ...
    if flag:
        ...
        print(c.kind, c.spelling)
    elif (c.kind == cl.CursorKind.BINARY_OPERATOR or
          c.kind == cl.CursorKind.UNARY_OPERATOR):
        print(c.kind, "op", c.displayname, c.spelling,
              dir(c), [e.spelling for e in c.get_tokens()])
    elif c.kind == cl.CursorKind.UNEXPOSED_EXPR:
        print(c.kind, "unexp", c.spelling, [
              e.spelling for e in c.get_tokens()])
    elif c.kind == cl.CursorKind.DECL_REF_EXPR:
        print(c.kind, "ref -exp", c.spelling,
              [e.spelling for e in c.get_tokens()])
    else:
        print(c.kind)
